<div class="field buttonField <?= $fullWidth ? 'buttonField_fullWidth' : '' ?>">
	...
	<?php else: ?>
		<button type="<?= $sanitizer->entities1($type) ?>" class="btn <?= $sanitizer->entities1($btnClasses) ?>" name="<?= $sanitizer->entities1($name) ?>" <?= $sanitizer->entities1($attributeString) ?> value="<?= $sanitizer->entities1($value) ?>">
			<?php if($icon): ?>
				<i class="fa fa-fw fa-<?= $sanitizer->entities1($icon) ?>"></i>
			<?php endif ?>
			<?= $sanitizer->entities1($label) ?>
