		...
		<ul class="userChooser <?= $singular ? 'userChooser_singular' : '' ?> group">
			<?php foreach($options as $option): ?>
				<li class="userChooser-item left <?= in_array($option->id, $value) ? 'userChooser-selected' : '' ?>">
					...
					<input class="userChooser-checkbox" type="checkbox" name="<?= $sanitizer->entities1($name) ?>[]" value="<?= $sanitizer->entities1($option->id) ?>" <?= $value && in_array($option->id, $value) ? 'checked="checked"' : '' ?> <?= $disabled ? 'disabled' : '' ?> />
