<?php include('../Field/header.php') ?>
	...
	<?php if(!$readOnly): ?>
		<div class="field-input imageField-input">
			<input <?= $formAttribute ?> type="file" class="<?= $sanitizer->entities1($buttonClasses) ?>" name="<?= $sanitizer->entities1($name) ?>" <?php if($id): ?>id="input_<?= $sanitizer->entities1($id) ?>"<?php endif ?> <?= $disabled ? 'disabled' : '' ?> />
		</div>
	<?php endif ?>
