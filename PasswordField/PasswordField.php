	...
	<div class="gGrid">
		<?php if($confirmPassword): ?><div class="<?= $stackInputs ? 'span12' : 'span6 span12_600px' ?>"><?php endif ?>
			...
			<div class="passwordField-inputWrap">
				<input <?= $formAttribute ?> maxlength="<?= $sanitizer->entities1($maxLength) ?>" name="<?= $sanitizer->entities1($name) ?>[]" <?php if($id): ?>id="input_<?= $sanitizer->entities1($id) ?>"<?php endif ?> class="field-input txtBox <?= $sanitizer->entities1($error) ? 'txtBox_error' : '' ?>" type="password" placeholder="<?= $sanitizer->entities1($placeholder) ?>" value="<?= $showValue ? $sanitizer->entities($value[0]) : '' ?>" <?= $disabled ? 'disabled' : '' ?> />
		...
		<?php if($confirmPassword): ?>
			<div class="<?= $stackInputs ? 'span12' : 'span6 span12_600px' ?>">
				...
				<input <?= $formAttribute ?> maxlength="<?= $sanitizer->entities1($maxLength) ?>" name="<?= $sanitizer->entities1($name) ?>[]" <?php if($id): ?>id="input_<?= $sanitizer->entities1($id) ?>Confirm"<?php endif ?> class="field-input txtBox <?= $error ? 'txtBox_error' : '' ?>" type="password" placeholder="<?= $sanitizer->entities1($placeholderConfirm) ?>" value="<?= $showValue ? $sanitizer->entities($value[1]) : '' ?>" <?= $disabled ? 'disabled' : '' ?> />
			</div>
		<?php endif ?>
	</div>
