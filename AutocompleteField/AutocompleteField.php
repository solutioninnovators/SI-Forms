<?php include('../Field/header.php') ?>
	...
	<div class="autocompleteField-inputs">
		...
		<input maxlength="<?= $sanitizer->entities1($maxLength) ?>" id="input_<?= $sanitizer->entities1($id) ?>" class="field-input txtBox <?= $error ? 'txtBox_error' : '' ?>" type="text" placeholder="<?= $sanitizer->entities1($placeholder) ?>" value="<?= $sanitizer->entities1($displayValue) ?>" <?= $disabled ? 'disabled' : '' ?> />
