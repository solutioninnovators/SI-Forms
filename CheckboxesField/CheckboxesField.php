	...
	<ul class="checkboxesField-list <?= $columnize ? 'checkboxesField-columnize' : '' ?>">
		<?php foreach($options as $option): ?>
			<li class="<?= $columnize ? 'dontsplit' : '' ?>">
				<label class="checkboxesField-label"><input type="checkbox" name="<?= $sanitizer->entities1($name) ?>[]" value="<?= $sanitizer->entities1($option['value']) ?>" <?= $value && in_array($option['value'], $value) ? 'checked="checked"' : '' ?> <?= $disabled ? 'disabled' : '' ?> /> <?= $sanitizer->entities1($option['label']) ?></label>
			</li>
		<?php endforeach ?>
	</ul>
